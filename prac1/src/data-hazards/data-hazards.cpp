...
void calculate(float& result, size_t total_iterations) {
  ...
  PRAGMA_UNROLL(1) // don't unroll
  for (size_t j = 0; j < total_iterations; ++j) {
    PRAGMA_UNROLL(256) // unroll 256 times (or as much as possible, i.e, num_vars times)
    for (int k = 0; k < num_vars; ++k) {
      a[k] = _mm_add_ss(a[k], one);
    } 
  }
  for (size_t i = 1; i < num_vars; ++i) {
    a[0] = _mm_add_ss(a[0], a[i]);
  }
  _mm_store_ss(&result, a[0]);
}
